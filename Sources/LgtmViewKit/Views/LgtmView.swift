...
final class LgtmStore: ObservableObject {
    ...
    /// Which face of the stack is showing. Owned here rather than as `@State`
    /// on `LgtmRootView` so `FindingCard`'s "Show pods" (Findings tab) can
    /// switch to Cluster without a nav stack or environment object to reach
    /// through - it already holds a reference to this store.
    @Published var tab: LgtmTab {
        didSet {
            guard tab != oldValue else { return }
            UserDefaults.standard.set(tab.rawValue, forKey: LgtmTab.key(context))
        }
    }

    /// Set by `FindingCard.showPods()`: the component to scroll to once the
    /// Cluster tab is back on screen. `LgtmClusterView` consumes and clears
    /// it - see `consumeScrollTarget` there.
    @Published var scrollToComponentTitle: String?

    let context: String
    /// Live pod/deployment state for the Cluster tab, refreshed alongside the
    /// analyser report in `load()` below so the tab's two halves never show
    /// two different moments in time. See `LgtmClusterStore`'s own doc
    /// comment for why this isn't `ClusterStore`.
    let clusterStore: LgtmClusterStore

    private init(context: String) {
        self.context = context
        self.window = LgtmWindow.stored(for: context)
        self.tab = LgtmTab.stored(for: context)
        self.clusterStore = LgtmClusterStore(context: context)
        #if DEBUG
    ...
    func load() async {
        ...
        // Runs alongside the two analyser passes rather than after them - the
        // Cluster tab's live state and the report's classification are two
        // independent fetches, and there is no reason to pay for them serially.
        async let clusterLoad: Void = clusterStore.load()
...
struct LgtmRootView: View {
    ...
    private var tabbed: some View {
        VStack(spacing: 0) {
            ...
            switch lgtm.tab {
            case .cluster:
                LgtmClusterView(report: lgtm.classification, store: lgtm.clusterStore,
                                 scrollToComponentTitle: $lgtm.scrollToComponentTitle)
            case .metrics:
            ...
            case .findings:
                if let full = lgtm.full {
                    LgtmFindingsView(report: full, lgtm: lgtm)
                } else {
                    pending("Findings")
                }
            }
...
struct LgtmFindingsView: View {
    ...
    @ObservedObject var lgtm: LgtmStore
        ...
        return VStack(alignment: .leading, spacing: 10) {
            ...
            ForEach(flagged) { component in
                ForEach(component.findings) { finding in
                    FindingCard(component: component, finding: finding, lgtm: lgtm)
                }
            }
        }
    }
...
private struct FindingCard: View {
    ...
    /// out; this switches to the Cluster tab and scrolls to this component's
    /// card - `LgtmClusterComponentCard` already shows every one of its pods,
    /// and from there Logs/Describe are one tap away via `PodInspectSheet`.
    /// Not a sheet itself: a finding is about a whole component (every pod
    /// under it), while `PodInspectSheet` only ever inspects one pod.
    private func showPods() {
        lgtm.scrollToComponentTitle = component.title
        lgtm.tab = .cluster
    }
