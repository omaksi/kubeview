...
public enum PodDetailTab: String, CaseIterable, Identifiable {
...
public struct PodLogsView: View {
    let route: PodRoute
    ...
    public init(route: PodRoute, containers: [String], context: String) {
        self.route = route
...
public struct PodDescribeView: View {
    let route: PodRoute
    let context: String
    @StateObject private var loader: PodDescribeLoader
    @State private var filter: String = ""

    public init(route: PodRoute, context: String) {
        self.route = route
        self.context = context
