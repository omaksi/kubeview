...
@MainActor
public final class EmojiStore: ObservableObject {
    @Published private var map: [String: String] = [:]
    private let defaultsKey = "kubeview.emojiMap"

    public init() { load() }

    public func emoji(for ref: ResourceRef) -> String? { map[ref.storageKey] }

    public func set(_ emoji: String?, for ref: ResourceRef) {
