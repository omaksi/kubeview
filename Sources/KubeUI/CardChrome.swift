...
public struct ResourceCard<Content: View>: View {
    let ref: ResourceRef
    let navigable: Bool
    let dimmed: Bool
    let context: String
    ...
    public init(ref: ResourceRef, navigable: Bool = false, dimmed: Bool = false, context: String, @ViewBuilder content: () -> Content) {
        self.ref = ref
...
public struct ResourceTitle: View {
    let ref: ResourceRef
    let name: String
    var font: Font = .system(.callout, design: .monospaced).weight(.semibold)
    @EnvironmentObject var emojis: EmojiStore

    public init(ref: ResourceRef, name: String, font: Font = .system(.callout, design: .monospaced).weight(.semibold)) {
        self.ref = ref
        self.name = name
        self.font = font
    }
...
public struct DescribeSheet: View {
    let ref: ResourceRef
    let context: String
    ...
    public init(ref: ResourceRef, context: String, isOpen: Binding<Bool>) {
        self.ref = ref
        self.context = context
        self._isOpen = isOpen
